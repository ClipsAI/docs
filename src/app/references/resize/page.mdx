...
To resize the video using the returned **crops**, you'll first need to [install ffmpeg](https://github.com/kkroening/ffmpeg-python/tree/master?tab=readme-ov-file#installing-ffmpeg). Note that this is a command line tool, *not* a python library. Once done, simply run the following code.
